...
export const TodoList = ({ id, title } : {id: number, title: string}) => {
  ...
  return (
    <div className="bg-white rounded shadow p-6 m-4 w-full lg:w-3/4 lg:max-w-lg">
      <div className="mb-4">
        <h1 className="text-grey-darkest">{ title } <span>({completedTodo.length}/{todos.length})</span></h1>
...
const TodoItem = ({todo, toggleTodoDone, handleRemoveTodo}: TodoItemProps) => {
  const doneStyle = todo.completed ? 'text-gray-400 line-through' : 'text-gray-900'
    ...
    <div className={"flex justify-between items-center border-b-2"}>
      <div className={`flex items-center py-2 flex-grow  ${doneStyle}`}
        ...
        <input
          ...
          readOnly
        />
        <label htmlFor={"todo-" + todo.id} className="ms-2 text-md font-medium pointer-events-none">{todo.title}</label>
      </div>
